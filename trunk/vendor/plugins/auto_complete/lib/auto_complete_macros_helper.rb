module AutoCompleteMacrosHelper      
  ...
  def auto_complete_manager_result(entries, field, phrase = nil)
    return unless entries
    items = entries.map { |entry|
      begin
        geo_name = Geography.find(entry['geography_id']).blank? ? ' ' : h(Geography.find(entry['geography_id']).name)
      rescue
        geo_name ="n/a"
      end
      begin
        asset_name = Asset.find(entry['asset_primary_id']).blank? ? ' ' : h(Asset.find(entry['asset_primary_id']).name)
      rescue
        asset_name ="n/a"
      end
      content_tag("li", "<div class='name'>#{h(entry['name'])}</div>"+"<label style='font-size:10px;'>[<b>Asset:</b> #{asset_name}, <b>Geography:</b> #{geo_name}]</label>")
    }
    content_tag("ul", items.uniq)
  end
